...
impl MarkovChain {
    ...
    fn update_states(&mut self, key : Vec<String>, val : &str) {
    ...
    fn choose_transition(&self, key : &Vec<String>) -> String{
    ...
    // create_sentence
    fn create_sentence(&self) -> Vec<String> {
    ...
    // a more usable form.
    fn process_training_line(s : String) -> String {
    ...
    // training data.
    fn add_training_data<R : BufRead> (&mut self, reader : &mut R) {
        let mut key = self.start_state();
    ...
    fn start_state(&self) -> Vec<String> {
        let mut key = vec![];
